...
public class Book {
	...
	public void addToTheListOfDeposits(Deposit d) {
		if(deposits.isEmpty()) {
			...
			for(Deposit dep: deposits) {
				if(dep.getDepositID() == d.getDepositID()) {
	...
	public Collection<Deposit> findDepositDetails(String location) {
		return this.deposits.stream().filter(d -> d.getLocation().equals(location)).collect(Collectors.toList());
	}
